...
BestInPlaceEditor.prototype = {
    ...
    update: function () {
        ...
        if (this.formType in {"input": 1, "textarea": 1, "select": 1} && value === this.oldValue) {
            this.abort();
            return true;
        } else if (this.formType in {"select": 1} && (this.collectionValue === value || this.collectionValue && (value === this.collectionValue.toString()))) {
            this.abort();
            return true;
        } else if (this.formType in {"select": 1} && (this.element && this.element.attr('data-bip-value') && (value === this.element.attr('data-bip-value').toString()))) {
            this.abort();
            return true;
        }
